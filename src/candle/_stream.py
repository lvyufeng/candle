try:
    ...
    if exc.name in {"candle._cython._stream", "candle._cython"}:
...
if not _STREAM_CYTHON:
    ...
    class PyTorchFileWriter:
        ...
        Wraps :class:`PyTorchStreamWriter` and exposes the same snake_case
        methods that the C++ pybind11 bindings register.
        """

        def __init__(self, file_name_or_writer, *, compress: bool = False):
            self._writer = PyTorchStreamWriter(file_name_or_writer)
            self._compress = compress

        def write_record(self, name: str, data, size: int = None) -> None:
            if size is None:
                self._writer.writeRecord(name, data, compress=self._compress)
            else:
                self._writer.writeRecord(name, data, size=size, compress=self._compress)

        def write_end_of_file(self) -> None:
            self._writer.writeEndOfFile()

        def set_min_version(self, version: int) -> None:
            self._writer.setMinVersion(version)

        def get_all_written_records(self):
            return self._writer.getAllWrittenRecords()
